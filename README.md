...
[![Harsh Kedia — fast, quiet software for loud problems · harshkedia.com](./banner.png)](https://harshkedia.com)
...
[![X](https://img.shields.io/badge/X-0a0a0a?style=flat-square&logo=x&logoColor=00ff94)](https://x.com/harshkedia717)
[![Email](https://img.shields.io/badge/email-0a0a0a?style=flat-square&logo=gmail&logoColor=00ff94)](mailto:harshkedia717@gmail.com)
...
`Python` `TypeScript` `Go` `React` `Next.js` `FastAPI` `Node.js` `gRPC`
`RAG` `Agents` `MCP` `LangGraph` `PyTorch` `evals` `LLM tracing` `guardrails`
`PostgreSQL` `Redis` `Kafka` `Milvus` `Elasticsearch`
`Kubernetes` `Docker` `AWS` `Azure` `GCP` `Cloudflare` `OpenTelemetry`
